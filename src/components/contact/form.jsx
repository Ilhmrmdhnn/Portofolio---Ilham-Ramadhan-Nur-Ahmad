...
const Form = () => {
  ...
  const notify = () => toast(`Hi ${name } Hello, thanks for your email`, {
    position: "top-right",
    autoClose: 5000,
  ...
  const emailHandler = (e) => {
    ...
    emailjs.send('service_2940vyo', 'template_5wg5pbk', {
      from_name: name,
      to_name: 'Ilham Ramadhan Nur Ahmad',
      message,
      reply_to: email
  ...
  return (
    <div className="flex w-full flex-col bg-white dark:bg-button2 rounded-lg px-4 py-10 gap-y-6 mb-10 lg:mb-0 sm:w-4/5 lg:order-2 lg:bg-button dark:lg:bg-white">
      <h2 className="text-title dark:text-dtitle text-2xl dark:lg:text-title">Hello</h2>
      <form onSubmit={emailHandler.bind(this)} className="flex flex-col gap-y-6">
        ...
        <input type="submit" className="bg-dbutton cursor-pointer text-button border border-transparent lg:bg-white dark:lg:bg-pink-shadow-300 hover:dark:bg-button lg:border-zinc-900 hover:bg-button hover:text-button2 duration-300 rounded-lg py-3" value="Send" />
      </form>
